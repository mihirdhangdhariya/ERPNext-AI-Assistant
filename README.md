...
```python
User: "Show sales for Tech Innovations"
→ Recalls "Tech Innovations is client C-1001"
```
...
**Data Flow:**  
1. User query → Department-specific agent  
2. Context retrieval from FAISS  
3. ERP operation execution  
4. Response formatting → User  
5. Interaction storage → FAISS  
...
| Challenge                        | Solution                                                    |
|----------------------------------|--------------------------------------------------------------|
| ERPNext API Limitations          | Built mock operations layer with dynamic data generation     |
| LLM Hallucinations               | Structured output constraints + parameter validation wrappers|
| Context Window Limitations       | HNSW-based vector search for relevant history retrieval       |
| Error Handling                   | Self-correction system with retry mechanism                  |
...
- Edge case testing for 50+ natural language variants  
...
**Demo Access:** [Loom Demo](https://www.loom.com/share/9d319950a8fa42b3bfa59f48b34b42f3)